...
import (
	...
	"github.com/tskorvan/casbin/errors"
...
// domain is a prefix to the roles.
func (rm *RoleManager) HasLink(name1 string, name2 string, domain ...string) (bool, int, error) {
	if len(domain) == 1 {
		name1 = domain[0] + "::" + name1
		name2 = domain[0] + "::" + name2
	} else if len(domain) > 1 {
	...
	if name1 == name2 {
		return true, rm.maxHierarchyLevel, nil
	}
	...
	role1 := rm.createRole(name1)
	has, level := role1.hasRole(name2, rm.maxHierarchyLevel)
	return has, level, nil
}
...
func (r *Role) hasRole(name string, hierarchyLevel int) (bool, int) {
	if r.name == name {
	...
	for _, role := range r.roles {
		if has, level := role.hasRole(name, hierarchyLevel-1); has {
			return true, level
		}
	}
	return false, hierarchyLevel
